...
export default function Home({ navigation }) {
  ...
  useEffect(() => {
    ...
    axios.get('http://10.0.2.2:8000/usuarios/1').then((resposta) => {
    ...
    <View style={{ height: altura * 1, width: largura * 1, backgroundColor: '#C1D4EF', justifyContent: 'center', alignItems: 'center' }}>
      ...
      <View style={{ backgroundColor: '#D7E3F5', width: largura * 0.90, height: altura * 0.10, borderRadius: 20, marginBottom: altura * 0.015, flexDirection: 'row', alignContent: 'center', marginTop: altura * 0., elevation: 10, shadowColor: 'black', justifyContent:'center' }}>
        ...
        {viewSaldo ? (
          ...
          <Text style={{ color: 'white', fontSize: altura * 0.058, marginLeft: largura * 0.05 }}>------</Text>
        )}
      ...
      <View style={{ backgroundColor: '#D7E3F5', width: largura * 0.90, height: altura * 0.10, borderRadius: 20, marginBottom: altura * 0.015, display: 'flex', justifyContent: 'center', alignItems: 'center', elevation: 10 }}>
        <View style={{ display: 'flex', flexDirection: 'row', justifyContent: 'center', alignItems: 'center', height: altura * 0.075, width: largura * 1, marginTop: altura * 0.01 }}>
          <TouchableOpacity
            style={{ height: altura * 0.1, width: largura * 0.15, borderRadius: 1000, marginRight: largura * 0.1, backgroundColor: 'transparent', alignItems: 'center', justifyContent: 'center', marginTop: altura * 0.014 }}
            onPress={() => navigation.navigate('inseresaldo')}
          >
      ...
      <View style={{ backgroundColor: '#D7E3F5', width: largura * 0.95, height: altura * 0.60, borderRadius: 20, alignItems: 'center', elevation: 10 }}>
        <ScrollView contentContainerStyle={{ alignItems: 'center', flexGrow: 1 }}>
          ...
          {dados.map((card) => (
            <TouchableOpacity onPress={()=>navigation.navigate('vacacompleta',{id:card.id_vaquinha})}>
                <CardVaquinha key={card.id} nome={card.nome} meta={card.objetivo} valor={card.valor} />
            </TouchableOpacity>
          ))}
        </ScrollView>
      </View>
